		...
		foreach($this->query as $row_wall){
			...
			if($row_wall['attach']){
				...
				foreach($attach_arr as $attach_file){
					...
					//APPS
					} elseif ($attach_type[0] == 'apps'){

						$attach_not = 'no_apps.gif';

						if($attach_not == $attach_type[2]){

							$app_img = "/images/no_apps.gif";

						} else {

							$app_img = "/uploads/apps/{$attach_type[1]}/{$attach_type[2]}";	

						}

						$attach_result .= '<a href="/app'.$attach_type[1].'" onclick="Page.Go(this.href); clear_style(); return false;"><div class="apps_wall"><img src=\"'.$app_img.'\" /></div></a>';
					...
					//Видео
						...
						if($cnt_attach_video == 1 AND preg_match('/(photo|photo_u)/i', $row_wall['attach']) == false){
							...
							$row_video['video'] = strtr($row_video['video'], array('width="770"' => 'width="400"', 'height="420"' => 'height="290"'));
							
							$attach_result .= "<div class=\"cursor_pointer clear\" id=\"no_video_frame{$video_id}\" onClick=\"$('#'+this.id).hide();$('#video_frame{$video_id}').show();\">
							<div class=\"video_inline_icon\"></div><img src=\"/uploads/videos/{$attach_type[3]}/{$attach_type[1]}\" style=\"margin-top:3px\" width=\"400\" height=\"290\" /></div><div id=\"video_frame{$video_id}\" class=\"no_display\" style=\"padding-top:3px\">{$row_video['video']}</div><div class=\"video_inline_vititle\"></div><a href=\"/video{$attach_type[3]}_{$attach_type[2]}\" onClick=\"videos.show({$attach_type[2]}, this.href, location.href); return false\"><b>{$row_video['title']}</b></a>";
					...
					//Если ссылка
						...
						if(stripos($attach_type[4], '/uploads/attach/') === false){
							$attach_type[4] = '/images/no_ava_50.png';
							$no_img = false;
						} else
							$no_img = true;
			...
			if($row_wall['tell_uid']){
				if($row_wall['public'])
					$rowUserTell = $db->super_query("SELECT title, photo, adres FROM `".PREFIX."_communities` WHERE id = '{$row_wall['tell_uid']}'", false, "wall/group{$row_wall['tell_uid']}");
				else
					$rowUserTell = $db->super_query("SELECT user_search_pref, user_photo, alias FROM `".PREFIX."_users` WHERE user_id = '{$row_wall['tell_uid']}'");
				...
				$rowIn = $db->super_query("SELECT alias FROM `".PREFIX."_users` WHERE user_id = '{$row_wall['author_user_id']}'");
			
			if($rowIn['alias'])
				$Tell = $rowIn['alias'];
			else
				$Tell = 'id'.$row_wall['author_user_id'];
				...
				if($rowUserTell['alias'])
						$ava = '/uploads/users/'.$row_wall['tell_uid'].'/50_'.$rowUserTell['user_photo'];
					else
...
HTML;
			...
			if($rowIn['alias'])
				$tpl->set('{alias}', $rowIn['alias']);
			else
				$tpl->set('{alias}', 'id'.$row_wall['author_user_id']);
