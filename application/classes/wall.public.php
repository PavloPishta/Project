			...
			if($row_wall['attach']){
				...
				foreach($attach_arr as $attach_file){
					...
					//Видео
						...
						$attach_result .= "<div align=\"left\" style=\"margin-top:0px;margin-right:3px\"><a href=\"/video{$attach_type[3]}_{$attach_type[2]}\" onClick=\"videos.show({$attach_type[2]}, this.href, location.href); return false\"><img src=\"/uploads/videos/{$attach_type[3]}/{$attach_type[1]}\" onerror=\"this.src='/images/video_error.gif'\" style=\"margin-top:3px;margin-right:3px\" align=\"left\" /></a></div>";
					...
					//Музыка
						...
						if($audioInfo){
							$jid++;
							$attach_result .= '<div class="audioForSize'.$row_wall['id'].'" id="audioForSize"><div class="audio_onetrack audio_wall_onemus"><div class="audio_playic cursor_pointer fl_l" onClick="music.newStartPlay(\''.$jid.'\', '.$row_wall['id'].')" id="icPlay_'.$row_wall['id'].$jid.'"></div><div id="music_'.$row_wall['id'].$jid.'" data="'.$audioInfo['url'].'" class="fl_l" style=\"margin-top: -1px;overflow: hidden;height: 15px;width: 330px;\"><a href="/?go=search&type=5&query='.$audioInfo['artist'].'&n=1" onClick="Page.Go(this.href); return false" title="'.stripslashes($audioInfo['artist']).' &ndash; '.stripslashes($audioInfo['name']).'"><b>'.stripslashes($audioInfo['artist']).'</b></a> &ndash; '.stripslashes($audioInfo['name']).'</div><div id="play_time'.$row_wall['id'].$jid.'" class="color777 fl_r no_display" style="margin-top:2px;margin-right:5px">00:00</div><div class="player_mini_mbar fl_l no_display player_mini_mbar_wall" id="ppbarPro'.$row_wall['id'].$jid.'"></div></div></div>';
						}
...
HTML;
			...
			if($row_wall['comments'] OR $public_admin){
				if($row_wall['fasts_num']){
					...
					$tpl->result[$this->compile] .= '<div id="wall_fast_block_'.$row_wall['id'].'" class="str_wall_rec_comments">';
