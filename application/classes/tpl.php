...
class mozg_template{
	...
	var $result = array ('info' => '', 'vote' => '', 'fm_wrap_bar' => '', 'content' => '' );
	...
	function load_template($tpl_name) {
		...
		if( strpos( $this->template, "{include file=" ) !== false ) {
			...
			$this->template = preg_replace( "#\\{include file=['\"](.+?)['\"]\\}#ies", "\$this->load_file('\\1', 'html')", $this->template );
	...
	function load_file( $name, $include_file = "html" ) {
		...
		if ($include_file == "php") {
			...
			if ($type != "php") return "To connect permitted only files with the extension: .html or .php";
			...
			if ( stristr ( dirname ($url['path']) , "html" ) !== false )
				return "Include files from directory /html/ is denied";
