...
if(isset($_POST['save'])){
	...
	if(strlen(trim($editedxfield[0])) > 0 AND strlen(trim($editedxfield[1])) > 0){
		...
		if($editedxfield[3] == "select" OR $editedxfield[3] == "checkbox"){
...
if($_GET['act'] == 'edit'){
	...
	if($editedxfield[2] == 'text') $selected1 = 'selected'; else $selected1 = '';
	if($editedxfield[2] == 'textarea') $selected2 = 'selected'; else $selected2 = '';
	
	if($editedxfield[2] == 'select') { 
		$selected3 = 'selected'; 
		$disp = '';
	} elseif($editedxfield[2] == 'checkbox') { 
		$selected4 = 'selected'; 
		$disp = '';
	} else {
		$disp = 'display:none';
...
<div class="fllogall" style="width:150px">Тип поля:</div>
 <select name="editedxfield[3]" class="inpu" onChange="if(this.value == 'select' || this.value == 'checkbox') document.getElementById('t').style.display = 'block'; else {document.getElementById('t').style.display = 'none';document.getElementById('6_select').value = '';}">
  <option value="text" {$selected1}>Одна строка</option>
  <option value="textarea" {$selected2}>Несколько строк</option>
  <option value="select" {$selected3}>Список</option>
  <option value="checkbox" {$selected4}>Несколько вариантов</option>
 </select>
<div class="mgcler"></div>
...
foreach($xfields as $name => $value){
	...
	elseif($value[2] == 'checkbox') $type = 'Несколько вари.';
	else $type = 'Список';
...
<input type="text" value="[xfgiven_{$value[0]}]<div class=&quot;flpodtext&quot;>{$value[1]}:</div> <div class=&quot;flpodinfo&quot;>[xfvalue_{$value[0]}]</div>[/xfgiven_{$value[0]}]" class="inpu" style="padding:2px;margin-bottom:-1px;margin-top:-5px;width:150px" onClick="this.select()" />
</div>
HTML;
