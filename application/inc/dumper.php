...
$lang = array('dumper_1'				=>	"Бэкап БД успешно создан в папке /application/backup/",
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										...
										if ($config['charset'] == "utf-8" AND $charset[2] == "utf-8" ) { $convert=true; $charset[2] = "utf8"; $this->restore_charset = "utf8"; }
...
.unterline {
	background: url(application/skins/images/line_bg.gif);
	width: 100%;
	height: 9px;
	font-size: 3px;
	font-family: tahoma;
...
.hr_line {
	background: url(application/skins/images/line.gif);
	width: 100%;
	height: 7px;
	font-size: 3px;
	font-family: tahoma;
...
STYLE="background: #5c9ccc url(application/inc/images/ui-bg_gloss-wave_55_5c9ccc_500x100.png) 50% 50% repeat-x;";
