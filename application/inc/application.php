...
if(isset($_POST['saveconf'])){
	...
	$handler = fopen(APPLICATION_DIR.'/data/config.php', "w");
	...
	msgbox('Настройки сохранены', 'Настройки системы были успешно сохранены!', '?mod=application');
} else {
	echoheader();
	echohtmlstart('Общие настройки');

	//Чтение всех языков
	$root_dir2 = scandir(APPLICATION_DIR.'./lang/');
...
<div class="fllogall">Стоимость 1 y.e(руб):</div><input type="text" name="save[cost_balance]" class="inpu" value="{$config['cost_balance']}" /><div class="mgcler"></div>
HTML;
