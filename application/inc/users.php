...
if($se_uid OR $sort OR $se_name OR $se_email OR $ban OR $delet OR $_GET['regdate']){
	...
	if($ureal){$where_sql .= "AND user_real = 1 ";$checked_ureal = "checked";}
	...
	else if($sort == 4) $order_sql = "`user_rate` DESC";
	else $order_sql = "`user_reg_date` DESC";
} else
	$order_sql = "`user_reg_date` DESC";
	
$selsorlist = installationSelected($sort, '<option value="1">по алфавиту</option><option value="2">по дате регистрации</option><option value="3">по дате посещения</option><option value="4">по рейтингу</option>');
...
$sql_ = $db->super_query("SELECT user_group, user_search_pref, user_id, user_reg_date, user_last_visit, user_email, user_delet, user_ban, user_balance, user_real, user_rate FROM `".PREFIX."_users`  {$where_sql} ORDER by {$order_sql} LIMIT {$limit_page}, {$gcount}", 1);
...
<div class="fllogall">Верификация:</div>
 <input type="checkbox" name="user_real" style="margin-bottom:10px" {$checked_ureal} />
<div class="mgcler"></div>
...
foreach($sql_ as $row){
	...
	$users .= <<<HTML
<div style="background:#fff;float:left;padding:5px;width:170px;text-align:center;font-weight:bold;" title="Баланс: {$row['user_balance']} голосов"><a href="/id{$row['user_id']}" target="_blank" style="{$color}">{$row['user_search_pref']}</a></div>
...
<div style="background:#fff;float:left;padding:5px;width:100px;text-align:center;margin-left:1px">{$row['user_real']}</div>
...
<div style="background:#f0f0f0;float:left;padding:5px;width:100px;text-align:center;font-weight:bold;margin-top:-5px;margin-left:1px">Верификация</div>
...
<font color="orange">Реальные люди помечены оранжевым цветом</font>
</div>
...
<select name="mass_type" class="inpu" style="width:260px">
 ...
 <option value="18">Поставить галочку "Реальный человек"</option>
 <option value="19">Снять галочку "Реальный человек"</option>
</select>
