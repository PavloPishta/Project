...
if($_GET['act'] == 'edit'){
	...
	$row = $db->super_query("SELECT title, descr, comments, photo, preal FROM `".PREFIX."_communities` WHERE id = '".$id."'");
	if($row){
		if(isset($_POST['save'])){
...
foreach($sql_ as $row){
	...
	$users .= <<<HTML
<div style="background:#fff;float:left;padding:5px;width:100px;text-align:center;"><a href="/id{$row['real_admin']}" target="_blank">{$row['user_name']}</a></div>
...
<select name="mass_type" class="inpu" style="width:260px">
 ...
 <option value="7">Сделать рекомендуемым</option>
 <option value="8">Убрать из рекомендуемых</option>
</select>
