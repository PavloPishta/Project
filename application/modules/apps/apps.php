...
if($logged){
	...
	switch($act){
		...
		case "view":
			...
			$tpl->load_template('apps/viewapplication.html');
		...
		//top tabs bar
		$tpl->load_template('apps/apps_category_top.html');
		...
		$tpl->load_template('apps/apps_category.html'); //Подключаем сам шаблон..
			...
			foreach($db_ads_apps as $ads_apps){
				...
				$tpl->load_template('apps/apps_right_ads.html');
				...
				$tpl->load_template('apps/cat_content.html');
		...
		case "app":
			...
			if($row['status'] == '1' or $row['user_id'] == $user_id){
				...
				if($row['url'] == '' && $row['flash'] == '')  $tpl->load_template('apps/editapp/no_app.html'); else $tpl->load_template('apps/application.html');

			} else $tpl->load_template('apps/editapp/ofline.html');
		...
		case "show_settings":
			...
			$tpl->load_template('/apps/show_settings.html');
		...
		case"doload":
			...
			$tpl->load_template('apps/my_application.html');
		...
		$tpl->load_template('apps/newapplication.html');
		...
		$tpl->load_template('apps/slider.html');
		...
		$tpl->load_template('apps/friends_application.html');
		...
		$tpl->load_template('apps/content.html');
