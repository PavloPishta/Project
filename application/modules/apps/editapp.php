...
if($logged){
	...
	switch($act){
		...
		case "create":
			...
			$tpl->load_template('apps/editapp/create.html');
		...
		case "info":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('apps/editapp/info.html');

				$tpl->compile('content');

			} else {

				$tpl->load_template('apps/editapp/error.html');
		...
		case "options":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('apps/editapp/options.html');
		...
		case "load_flash":
			...
			$tpl->load_template('/apps/editapp/load_flash.html');
		...
		case "payments":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('apps/editapp/table.html');
				...
				foreach($row1  as $rowsd){
					...
					$tpl->load_template('apps/editapp/payments.html');
		...
		case "admins":
			...
			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				...
				$tpl->load_template('apps/editapp/admin_all.html');
				...
				$tpl->load_template('apps/editapp/admins.html');
		...
		case "del_admin_form":
			...
			if($app_hash == md5($id.'_'.$key)){
				...
				$tpl->set('{name}', $users['user_name']. ' '. $users['user_lastname']);
  				...
  				$tpl->load_template('apps/editapp/del_admin_form.html');
  		...
  		case "search_admin":
    		...
    		$row = $db->super_query("SELECT user_photo, user_id FROM `".PREFIX."_users` WHERE user_id = '{$user_id}'");
  			...
  			$tpl->load_template('apps/editapp/add_admin.html');
		...
		case "load_photo":
			...
			$tpl->load_template('/apps/editapp/load_photo.html');
