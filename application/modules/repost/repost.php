...
if($logged){
	...
	switch($act){
		...
		case "groups":
			...
			if($sel_group != $check_IdGR AND $myRow['cnt'] == false AND stripos($rowGroup['admin'], "id{$user_id}|") !== false AND $rowGroup['del'] == 0 AND $rowGroup['ban'] == 0){
				$row['text'] = $db->safesql($row['text']);
				$row['attach'] = $db->safesql($row['attach']);
		...
		case "groups_2":
			...
			if($sel_group != $row['public_id'] AND $myRow['cnt'] == false AND stripos($rowGroup['admin'], "id{$user_id}|") !== false AND $rowGroup['del'] == 0 AND $rowGroup['ban'] == 0){
		...
		default:
			...
			//Выводим сообщества
			$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS id, title FROM `".PREFIX."_communities` WHERE admin regexp '[[:<:]](id{$user_id})[[:>:]]' ORDER by `traf` DESC LIMIT 0, 50", 1);
			...
			$tpl->load_template('repost/send.html');
