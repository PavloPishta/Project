...
if($logged){
    ...
    switch($act){
		...
		$db_ads = $db->super_query("SELECT id, settings, description, links, link, views, category FROM `".PREFIX."_ads` WHERE user_id = '{$user_id}' ORDER by rand() ASC LIMIT {$limit_page}, {$gcount}", 1);
			...
			//top tabs bar
			$tpl->load_template('ads/ads_top.html');
		...
		$tpl->load_template('ads/ads_view_my.html');
		...
		case "create_ads":
			...
			$tpl->load_template('ads/ads_create.html');
    ...
    ';
		...
		$tpl->load_template('ads/ads_target.html');
		...
		if($db_ads_all){
			...
			$tpl->load_template('ads/ads_view_all.html');
