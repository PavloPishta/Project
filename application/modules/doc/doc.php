	...
	switch($act){
		...
		case "download";
			...
			if($row){
				...
				include APPLICATION_DIR . '/classes/download.php';
		...
		case "list":
			...
			if(!$page_cnt){
				...
				$tpl->load_template('doc/top_list.html');
				$tpl->set('{doc-num}', $rowUser['user_doc_num']);
				$tpl->compile('content');
				
			}
				
			$tpl->load_template('doc/doc_list.html');
			foreach($sql_ as $row){
			...
			if($rowUser['user_doc_num'] > 20){
				...
				$tpl->load_template('doc/bottom_list.html');
				$tpl->compile('content');
		...
		default:
			...
			if(!$page_cnt){
				...
				$tpl->load_template('doc/top.html');
				$tpl->set('{doc-num}', $rowUser['user_doc_num']);
				$tpl->compile('content');
			}
			
			$tpl->load_template('doc/doc.html');
			foreach($sql_ as $row){
			...
			if(!$page_cnt AND $rowUser['user_doc_num'] > 20){
				$tpl->load_template('doc/bottom.html');
				$tpl->compile('content');
			}
