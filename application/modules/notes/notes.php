...
if($logged){
	...
	switch($act){
		...
		case "add":
			...
			$user_fm_wrap_bar = $lang['add_new_note'];
			
			//Загруажем head заметок
			$tpl->load_template('notes/head.html');
			$tpl->set('[add]', '');
			$tpl->set('[/add]', '');
			...
			$tpl->load_template('notes/add.html');
			$tpl->compile('content');
		break;
		...
		case "save":
			...
			//Подключаем парсер
			include APPLICATION_DIR.'/classes/parse.php';
			$parse = new parse();

			$title = htmlspecialchars_decode(ajax_utf8(textFilter($_POST['title'], false, true)));
			$text = $parse->BBparse(htmlspecialchars_decode(ajax_utf8(textFilter($_POST['text']))));
		...
		case "preview":
			...
			if($text && $title){
				...
				$tpl->load_template('notes/preview.html');
		...
		case "edit":
			...
			$user_fm_wrap_bar = $lang['note_edit'];
			...
			if($row){
				...
				$tpl->load_template('notes/edit.html');
				$tpl->set('{note-id}', $note_id);
		...
		case "addcomment":
			...
			if(!$CheckBlackList AND $check AND isset($textcom) AND !empty($textcom)){
				if($check){
					...
					$tpl->load_template('notes/comment.html');
		...
		case "view":
			...
			if(!$CheckBlackList){
				if($row){
					...
					$user_fm_wrap_bar = $lang['notes_view'];
					...
					//Загружаем шаблон вывода полного просомтра заметки
					$tpl->load_template('notes/full.html');
					$tpl->set('{note-id}', $note_id);
					$tpl->set('{title}', stripslashes($row['title']));
					...
					$tpl->load_template('notes/addcomment.html');
					$tpl->set('{note-id}', $note_id);
					$tpl->compile('content');
		...
		default:
			...
			if(!$CheckBlackList){
				...
				if($owner){
					...
					$user_fm_wrap_bar = 'У '.gramatikName($author_info[0]).' <span id="notes_num">'.$owner['user_notes_num'].'</span> '.gram_record($owner['user_notes_num'], 'notes');;
					...
					if($get_user_id == $user_id){
						...
						$tpl->set('[/owner]', '');
						$tpl->set_block("'\\[not-owner\\](.*?)\\[/not-owner\\]'si","");
						$user_fm_wrap_bar = 'У Вас <span id="notes_num">'.$owner['user_notes_num'].'</span> '.gram_record($owner['user_notes_num'], 'notes');
					} else {
						$tpl->set('[not-owner]', '');
						$tpl->set('[/not-owner]', '');
						$tpl->set_block("'\\[owner\\](.*?)\\[/owner\\]'si","");
					...
					//Выводим
					if($sql_){
						$tpl->load_template('notes/short.html');
						foreach($sql_ as $row){
							if($owner['user_photo'])
