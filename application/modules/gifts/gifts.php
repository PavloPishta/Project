...
if($logged){
	...
	switch($act){
		...
		case "view":
			...
			echo "<div class=\"cover_edit_title\"><div class=\"buttonsprofile albumsbuttonsprofile onclic_bottonond\" style=\"height:20px;\">
			<div class=\"activetab\"><a  class=\"cursor_pointer\" onClick=\"gifts.box('{$for_user_id}', 1); return false\"><div><b>Разное</b></div></a></div><a  class=\"cursor_pointer\" onClick=\"gifts.box3('{$for_user_id}', 1); return false\"><div><b>Романтика</b></div></a><a  class=\"cursor_pointer\" onClick=\"gifts.box1('{$for_user_id}', 1); return false\"><div><b>Дружба</b></div></a><a  class=\"cursor_pointer\" onClick=\"gifts.box2('{$for_user_id}', 1); return false\"><div><b>День рождение</b></div></a><div class=\"fl_r\"><a  class=\"cursor_pointer\" onClick=\"gifts.box('{$for_user_id}', 1); return false\">Показать еще</a></div></div></div>";
			...
			foreach($sql_ as $gift){
				...
				$gift_price = $gift['price'].' '.gram_record($gift['price'], 'balance');

				echo "<a href=\"\" class=\"gifts_onegif\" onMouseOver=\"gifts.showgift('{$gift['img']}')\" onMouseOut=\"gifts.showhide('{$gift['img']}')\" class=\"cursor_pointer\" onClick=\"gifts.select('{$gift['img']}', '{$for_user_id}'); return false\"><img src=\"/uploads/gifts/{$gift['img']}.png\" /><div class=\"gift_count no_display\" id=\"g{$gift['img']}\">{$gift_price}</div></a>";
			}
			
			$row = $db->super_query("SELECT user_balance FROM `".PREFIX."_users` WHERE user_id = '{$user_id}'");
			
			$gift_nucount = 'У Вас <b>'.$row['user_balance'].' </b> '.gram_record($row['user_balance'], 'balance');
			
			echo "<style>#box_bottom_left_text{padding-top:6px;float:left}</style><script>$('#box_bottom_left_text').html('{$gift_nucount}');</script>";
		...
		case "viewfr":
			NoAjaxQuery();
			$for_user_id = intval($_POST['user_id']);
			echo "<div class=\"cover_edit_title\"><div class=\"buttonsprofile albumsbuttonsprofile onclic_bottonond\" style=\"height:20px;\"><a class=\"cursor_pointer\" onClick=\"gifts.box('{$for_user_id}', 1); return false\"><div><b>Разное</b></div></a><a class=\"cursor_pointer\" onClick=\"gifts.box3('{$for_user_id}', 1); return false\"><div><b>Романтика</b></div></a><div class=\"activetab\"><a class=\"cursor_pointer\" onClick=\"gifts.box1('{$for_user_id}', 1); return false\"><div><b>Дружба</b></div></a></div><a class=\"cursor_pointer\" onClick=\"gifts.box2('{$for_user_id}', 1); return false\"><div><b>День рождение</b></div></a><div class=\"fl_r\"><a class=\"cursor_pointer\" onClick=\"gifts.box1('{$for_user_id}', 1); return false\">Показать еще</a></div></div></div>";
		...
		case "viewhp":
			NoAjaxQuery();
			$for_user_id = intval($_POST['user_id']);
			echo "<div class=\"cover_edit_title\"><div class=\"buttonsprofile albumsbuttonsprofile onclic_bottonond\" style=\"height:20px;\"><a class=\"cursor_pointer\" onClick=\"gifts.box('{$for_user_id}', 1); return false\"><div><b>Разное</b></div></a><a class=\"cursor_pointer\" onClick=\"gifts.box3('{$for_user_id}', 1); return false\"><div><b>Романтика</b></div></a><a class=\"cursor_pointer\" onClick=\"gifts.box1('{$for_user_id}', 1); return false\"><div><b>Дружба</b></div></a><div class=\"activetab\"><a class=\"cursor_pointer\" onClick=\"gifts.box2('{$for_user_id}', 1); return false\"><div><b>День рождение</b></div></a></div></div></div>";
		...
		case "viewr":
			NoAjaxQuery();
			$for_user_id = intval($_POST['user_id']);
			echo "<div class=\"cover_edit_title\"><div class=\"buttonsprofile albumsbuttonsprofile onclic_bottonond\" style=\"height:20px;\"><a class=\"cursor_pointer\" onClick=\"gifts.box('{$for_user_id}', 1); return false\"><div><b>Разное</b></div></a><div class=\"activetab\"><a class=\"cursor_pointer\" onClick=\"gifts.box3('{$for_user_id}', 1); return false\"><div><b>Романтика</b></div></a></div><a class=\"cursor_pointer\" onClick=\"gifts.box1('{$for_user_id}', 1); return false\"><div><b>Дружба</b></div></a><a class=\"cursor_pointer\" onClick=\"gifts.box2('{$for_user_id}', 1); return false\"><div><b>День рождение</b></div></a></div></div>";
		...
		case "send":
			...
			if($gifts['price'] AND $user_id != $for_user_id){
				if($row['user_balance'] >= $gifts['price']){
					...
					$db->query("INSERT INTO `".PREFIX."_historytab` SET user_id = '{$user_id}', for_user_id = '{$for_user_id}', title='', type = '1', price='{$gifts['price']}', status = '-', date = '{$server_time}'");
						...
						if($row_owner['user_last_visit'] >= $update_time){
							...
							$db->query("INSERT INTO `".PREFIX."_updates` SET for_user_id = '{$for_user_id}', from_user_id = '{$user_info['user_id']}', type = '7', date = '{$server_time}', text = '{$action_update_text}', user_photo = '{$user_info['user_photo']}', user_search_pref = '{$user_info['user_search_pref']}', lnk = '/gifts{$for_user_id}'");
		...
		default:
			...
			$tpl->load_template('gifts/head.html');
			$tpl->set('{uid}', $uid);
			if($user_id == $uid){
				$tpl->set('[owner]', '');
				$tpl->set('[/owner]', '');
			...
			if($owner['user_gifts']){
				...
				$tpl->load_template('gifts/gift.html');
				foreach($sql_ as $row){
					$tpl->set('{id}', $row['gid']);
					...
					if($row['privacy'] == 1 OR $user_id == $row['from_uid'] OR $user_id == $uid AND $row['privacy'] != 3){
						...
						$tpl->set('[link]', '<a href="/id'.$row['from_uid'].'" class=\"cursor_pointer\" onClick="Page.Go(this.href); return false">');
						$tpl->set('[/link]', '</a>');
