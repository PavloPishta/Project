			...
			if($mass_type <= 20 AND $mass_type >= 1){
				foreach($massaction_users as $user_id){
					...
					//Добавление в реальные люди
						...
						$db->query("UPDATE `".PREFIX."_users` SET ureal = '1' WHERE user_id = '".$user_id."'");
						mozg_clear_cache_file('user_'.$user_id.'/profile_'.$user_id);
						}
						
					//Удаление
					else if($mass_type == 19){
						$db->query("UPDATE `".PREFIX."_users` SET ureal = '0' WHERE user_id = '".$user_id."'");
						mozg_clear_cache_file('user_'.$user_id.'/profile_'.$user_id);
						}
					...
					//Бан до смены имени
						...
						$db->query("UPDATE `".PREFIX."_users` SET user_banname = '1' WHERE user_id = '".$user_id."'");
				...
				//Бан до смены имени
					...
					msgbox('Бан', 'Пользователь был забанен до смены имени', '?mod=users');
