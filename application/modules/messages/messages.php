...
if($logged){
	...
	switch($act){
		...
		case "history":
			...
			if($sql_){
				$tpl->load_template('messages/history.html');
				foreach($sql_ as $row){
		...
		case "review":
			...
			$user_fm_wrap_bar = $lang['msg_view'];
			...
			if($mid){
				...
				//header сообщений
				$tpl->load_template('messages/head.html');
				$tpl->set('{mid}', $mid);
				...
				if($row){
					$tpl->load_template('messages/review.html');
...
HTML;
		...
		case "outbox":
			...
			$user_fm_wrap_bar = $lang['msg_outbox'];
			...
			if($sql_){
				$tpl->load_template('messages/message.html');
				foreach($sql_ as $row){
		...
		default:
			...
			//################### Вывод всех полученных сообщений ###################//
				...
				$user_fm_wrap_bar = $lang['msg_inbox'];
