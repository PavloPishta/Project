	...
	switch($act){
		...
		case "requests":
			...
			if($user_info['user_friends_demands'])
				$user_fm_wrap_bar = $user_info['user_friends_demands'].' '.gram_record($user_info['user_friends_demands'], 'friends_demands');
			else
				$user_fm_wrap_bar = $lang['no_requests'];
			
			//Верх
			$tpl->load_template('friends/head.html');
			...
			$tpl->set_block("'\\[general-friends\\](.*?)\\[/general-friends\\]'si","");
			...
			if($user_info['user_friends_demands']){
				...
				$tpl->load_template('friends/request.html');
				foreach($sql_ as $row){
		...
		case "online":
			...
			if(!$CheckBlackList){
				...
				if($online_friends['cnt'])
					$user_fm_wrap_bar = 'У '.$gram_name.' '.$online_friends['cnt'].' '.gram_record($online_friends['cnt'], 'friends_online');
				else
					$user_fm_wrap_bar = $lang['no_requests_online'];
				...
				if($sql_){
					$tpl->load_template('friends/friend.html');
					foreach($sql_ as $row){
						$user_country_city_name = explode('|', $row['user_country_city_name']);
						$tpl->set('{country}', $user_country_city_name[0]);
		...
		case "box":
			...
			if($sql_usSex){
				...
				if($count['cnt']){
					...
					$tpl->load_template('friends/box_friend.html');
					foreach($sql_ as $row){
						$tpl->set('{user-id}', $row['friend_id']);
						$tpl->set('{name}', $row['user_search_pref']);
		...
		break;
			...
			default:
				...
				if(!$CheckBlackList){
					...
					if($friends_sql['user_friends_num'])
						$user_fm_wrap_bar = 'У '.$gram_name.' <span id="friend_num">'.$friends_sql['user_friends_num'].'</span> '.gram_record($friends_sql['user_friends_num'], 'friends');
					else
						$user_fm_wrap_bar = $lang['no_requests'];
