	...
	switch($act){
		...
		case "new":
			...
			if(stripos($row['ulist'], "|{$user_id}|") !== false AND $row['discussion']){
				...
				$tpl->load_template('forum/new.html');
		...
		case "add_msg":
			...
			if($row AND !$row['status'] AND $row2['discussion']){
				...
				$tpl->load_template('forum/msg.html');
		...
		case "prev_msg":
			...
			$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.mid, muser_id, msg, mdate, tb2.user_search_pref, user_photo, user_last_visit FROM `".PREFIX."_communities_forum_msg` tb1, `".PREFIX."_users` tb2 WHERE tb1.muser_id = tb2.user_id AND tb1.fid = '{$id}' AND mid < '{$first_id}' ORDER by `mdate` ASC LIMIT {$start_limit}, {$limit}", 1);
		...
		case "savetitle":
			...
			if(stripos($row2['admin'], "id{$user_id}|") !== false AND $row2['discussion'])
				$public_admin = true;
			else
				$public_admin = false;
		...
		case "delmsg":
			...
			if(stripos($row3['admin'], "id{$user_id}|") !== false)
				$public_admin = true;
			else
				$public_admin = false;
		...
		case "view":
			...
			if($row AND $row2['discussion']){
				...
				$tpl->load_template('forum/view.html');
		...
		default:
			...
			if($row['discussion']){
				...
				//Верхушка
				if(!$_POST['a']){
					$tpl->load_template('forum/head.html');
					$tpl->set('{id}', $public_id);
				...
				if($sql_){
					...
					$tpl->load_template('forum/theme.html');
					foreach($sql_ as $row){
				...
				if(!$_POST['a'] AND $forum_num > 20){
					$tpl->load_template('forum/bottom.html');
					$tpl->set('{id}', $public_id);
					if(!$row['forum_num']) $row['forum_num'] = '';
					$tpl->set('{forum-num}', $row['forum_num']);
					$tpl->compile('content');
