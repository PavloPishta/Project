...
if($logged){
	...
	switch($act){
		...
		case "create":
			...
			if(isset($name) AND !empty($name)){
				...
				if($row['user_albums_num'] < $config['max_albums']){
					...
					mozg_clear_cache_file("user_{$user_info['user_id']}/photos");
		...
		case "create_page":
			NoAjaxQuery();
			$tpl->load_template('albums_create.html');
			$tpl->compile('content');
			AjaxTpl();
			die();
		break;
		...
		case "add":
			...
			if($row){
				...
				$user_fm_wrap_bar = $lang['add_photo_2'];
				$tpl->load_template('albums_addphotos.html');
				$tpl->set('{aid}', $aid);
					...
					if(in_array(strtolower($type), $allowed_files)){
						...
						if($image_size < $config['max_photo_size']){
							...
							if(move_uploaded_file($image_tmp, $album_dir.$image_rename.$res_type)){
								...
								mozg_mass_clear_cache_file('user_'.$user_id.'/photos');
		...
		case "del_photo":
			...
			if($row['user_id'] == $user_id){
				...
				mozg_mass_clear_cache_file("user_{$user_info['user_id']}/photos");
		...
		case "edit_page";
			...
			if($row){
				...
				$tpl->load_template('albums_edit.html');
				$tpl->set('{id}', $row['aid']);
		...
		case "edit_cover";
			...
			if($user_id AND $id){
				...
				if($sql_){
					...
					$tpl->load_template('albums_editcover_photo.html');
					foreach($sql_ as $row){
		...
		case "all_photos_box";
			...
			if($row_album['photo_num']){
				if($notes)
					$tpl->load_template('notes/attatch_addphoto_top.html');
				else
					$tpl->load_template('wall/attatch_addphoto_top.html');
				...
				if(!$notes)
					$tpl->load_template('albums_all_photos.html');
				else
					$tpl->load_template('albums_box_all_photos_notes.html');
...
HTML;
		...
		case "all_comments":
			...
			if($privacy AND !$CheckBlackList){
				...
				if($uid AND !$aid){
					$user_fm_wrap_bar = $lang['comm_form_album_all'];
					$metatags['title'] = $lang['comm_form_album_all'];
				} else {
					$user_fm_wrap_bar = $lang['comm_form_album'];
					$metatags['title'] = $lang['comm_form_album'];
				}
				
				//Загружаем HEADER альбома
				$tpl->load_template('albums_top.html');
				$tpl->set('{user-id}', $uid);
				$tpl->set('{aid}', $aid);
				...
				if($sql_){
					...
					$tpl->load_template('albums_comment.html');
					...
					$user_fm_wrap_bar = $row_album['all_comm_num'].' '.gram_record($row_album['all_comm_num'], 'comments');
				} else
					msgbox('', $lang['no_comments'], 'info_2');
			} else {
				$user_fm_wrap_bar = $lang['title_albums'];
		...
		case "edit_pos_photos":
			...
			if($check_album){
				...
				$user_fm_wrap_bar = $lang['editphotos'];
				...
				if($sql_){
					...
					$tpl->load_template('albums_editphotos.html');
					foreach($sql_ as $row){
		...
		case "view":
			...
			if(!$CheckBlackList){
				...
				if($album_privacy[0] == 1 OR $album_privacy[0] == 2 AND $check_friend OR $user_info['user_id'] == $row_album['user_id']){
					...
					$user_fm_wrap_bar = '<span id="photo_num">'.$row_album['photo_num'].'</span> '.gram_record($row_album['photo_num'], 'photos');

					if($sql_photos){
						$tpl->load_template('album_photo.html');
		...
		case "new_photos":
			...
			$user_fm_wrap_bar = 'Новые фотографии со мной';
			
			//Загрузка верхушки
			$tpl->load_template('albums_top_newphotos.html');
			...
			$tpl->load_template('albums_top_newphoto.html');
			if($sql_){
				foreach($sql_ as $row){
				...
				if(!$CheckBlackList){
					...
					if($sql_){
						...
						if($row_owner['user_albums_num']){
							if($user_info['user_id'] == $uid){
								$user_fm_wrap_bar = 'У Вас <span id="albums_num">'.$row_owner['user_albums_num'].'</span> '.gram_record($row_owner['user_albums_num'], 'albums');
							} else {
								$user_fm_wrap_bar = 'У '.gramatikName($author_info[0]).' '.$row_owner['user_albums_num'].' '.gram_record($row_owner['user_albums_num'], 'albums');
							}
						...
						$tpl->load_template('albums_info.html');
						//Показ скрытых тексто только для владельца страницы
						if($user_info['user_id'] == $uid){
							$tpl->set('[owner]', '');
							$tpl->set('[/owner]', '');
