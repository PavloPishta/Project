...
if($logged){
	...
	if($type == 1){ //Если критерий поиск "по людям"
		...
		$sql_query = "SELECT SQL_CALC_FOUND_ROWS id, photo, photo_original, title, add_date, comm_num, owner_user_id, descr, nocopy FROM `".PREFIX."_videos` WHERE title LIKE '%{$query}%' AND privacy = 1 AND nocopy !=1 ORDER by `add_date` DESC LIMIT {$limit_page}, {$gcount}";
		$sql_count = "SELECT COUNT(*) AS cnt FROM `".PREFIX."_videos` WHERE title LIKE '%{$query}%' AND privacy = 1 AND nocopy !=1";
	...
	//Head поиска
	$tpl->load_template('search/head.html');
	if($query)
	...
	$tpl->set("{activetab-{$type}}", 'onclic_botton');
	$tpl->set("{type}", $type);
	...
	if($sql_){
		...
		if($type == 1){
			$tpl->load_template('search/result_people.html');
			foreach($sql_ as $row){
		...
		//Если критерий поиск "по видеозаписям"
			...
			$tpl->load_template('search/result_video.html');
			foreach($sql_ as $row){
				...
				// Добавляем низ.
				if($gcount){
					$tpl->load_template('search/bottom.html');
					$tpl->compile('content');
				}

		//Если критерий поиск "по заметкам"
		} elseif($type == 3){
			$tpl->load_template('search/result_note.html');
			foreach($sql_ as $row){
				if($row['user_photo'])
		...
		//Если критерий поиск "по сообещствам"
			...
			$tpl->load_template('search/result_groups.html');
			foreach($sql_ as $row){
				if($row['photo'])
		...
		//Если критерий поиск "по аудизаписям"
			...
			$tpl->load_template('search/result_audio.html');
			$jid = 0;
			foreach($sql_ as $row){
				$jid++;
				$tpl->set('{jid}', $jid);
