				...
				if($row){
					...
					if($xPrivasy AND $user_id != $for_user_id){
						...
						if($row_owner['user_last_visit'] >= $update_time){
							$db->query("INSERT INTO `".PREFIX."_updates` SET for_user_id = '{$for_user_id}', from_user_id = '{$user_info['user_id']}', type = '8', date = '{$server_time}', text = '{$msg}', user_photo = '{$user_info['user_photo']}', user_search_pref = '{$user_info['user_search_pref']}', onk = '{$user_info['user_id']}|{$user_info['user_search_pref']}'");
							mozg_create_cache("user_{$for_user_id}/updates", 1);
						}

						//Ответ скрипта
						$tpl->load_template('im_chat/msg.html');
						$tpl->set('{ava}', $my_ava);
							...
							foreach($attach_arr as $attach_file){
								...
								//Если документ
									...
									if($row_doc){
										...
										$attach_result .= '<div style="margin-top:5px;margin-bottom:5px" class="clear"><div class="doc_attach_ic fl_l" style="margin-top:4px;margin-left:0px"></div><div class="attach_link_block_te"><div class="fl_l">'.$lang['lang_629'].' <a href="/index.php?go=doc&act=download&did='.$doc_id.'" target="_blank" onMouseOver="myhtml.title(\''.$doc_id.$cnt_attach.$dbid.'\', \'<b>'.$lang['lang_628'].' '.$row_doc['dsize'].'</b>\', \'doc_\')" id="doc_'.$doc_id.$cnt_attach.$dbid.'">'.$row_doc['dname'].'</a></div></div></div><div class="clear"></div>';
		...
		case "update":
			...
			if($sql_){
				...
				foreach($sql_ as $row){
					...
					if($row['pm_read'] == 'no'){
						$tpl->set('{new}', 'im_class_new');
						$tpl->set('{read-js-func}', 'onMouseOver="im_chat.read(\''.$row['id'].'\', '.$row['history_user_id'].', '.$user_id.')"');
					} else {
						$tpl->set('{new}', '');
						...
						foreach($attach_arr as $attach_file){
							...
							//Если ссылка
								...
								if($no_img AND $attach_type[2]){
									...
									$attach_result .= '<div style="margin-top:2px" class="clear"><div class="attach_link_block_ic fl_l" style="margin-top:4px;margin-left:0px"></div><div class="attach_link_block_te"><div class="fl_l">'.$lang['lang_212'].': <a href="/away.php?url='.$attach_type[1].'" target="_blank">'.$rdomain_url_name.'</a></div></div><div class="clear"></div><div class="wall_show_block_link" style="border:0px"><a href="/away.php?url='.$attach_type[1].'" target="_blank"><div style="width:108px;height:80px;float:left;text-align:center"><img src="'.$attach_type[4].'" /></div></a><div class="attatch_link_title"><a href="/away.php?url='.$attach_type[1].'" target="_blank">'.$str_title.'</a></div><div style="max-height:50px;overflow:hidden">'.$attach_type[3].'</div></div></div>';

									$resLinkTitle = $attach_type[2];
									$resLinkUrl = $attach_type[1];
								} else if($attach_type[1] AND $attach_type[2]){
									$attach_result .= '<div style="margin-top:2px" class="clear"><div class="attach_link_block_ic fl_l" style="margin-top:4px;margin-left:0px"></div><div class="attach_link_block_te"><div class="fl_l">'.$lang['lang_212'].': <a href="/away.php?url='.$attach_type[1].'" target="_blank">'.$rdomain_url_name.'</a></div></div></div><div class="clear"></div>';
							...
							//Если документ
								...
								if($row_doc){
									...
									$attach_result .= '<div style="margin-top:5px;margin-bottom:5px" class="clear"><div class="doc_attach_ic fl_l" style="margin-top:4px;margin-left:0px"></div><div class="attach_link_block_te"><div class="fl_l">'.$lang['lang_629'].' <a href="/index.php?go=doc&act=download&did='.$doc_id.'" target="_blank" onMouseOver="myhtml.title(\''.$doc_id.$cnt_attach.$row['id'].'\', \'<b>'.$lang['lang_628'].' '.$row_doc['dsize'].'</b>\', \'doc_\')" id="doc_'.$doc_id.$cnt_attach.$row['id'].'">'.$row_doc['dname'].'</a></div></div></div><div class="clear"></div>';
							...
							//Если опрос
								...
								if($vote_id){
									...
									else $answer_num_text = $lang['lang_587'];
									
									if($row_vote['answer_num'] <= 1) $answer_text2 = $lang['lang_636'];
									else $answer_text2 = $lang['lang_635'];
					...
					if($row['tell_uid']){
						...
						if(date('Y-m-d', $row['tell_date']) == date('Y-m-d', $server_time))
							$dateTell = langdate($lang['lang_643'].' в H:i', $row['tell_date']);
						elseif(date('Y-m-d', $row['tell_date']) == date('Y-m-d', ($server_time-84600)))
							$dateTell = langdate($lang['lang_640'].' в H:i', $row['tell_date']);
						else
...
HTML;
		...
		case "history":
			...
			if(!$first_id){
				$tpl->result['content'] .= '<div class="fm_hiden_scroll"><div class="im_scroll'.$for_user_id.' im_scroll">';
				if($count['all_msg_num'] > $limit_msg)
	
					$tpl->result['content'] .= '<div class="cursor_pointer" onClick="im_chat.page('.$for_user_id.'); return false" id="wall_all_records'.$for_user_id.'"><div class="str_wall_all_comm" id="load_wall_all_records'.$for_user_id.'" style="margin-left:0px">Показать предыдущие сообщения</div></div><div id="prevMsg'.$for_user_id.'"></div>';
					
				$tpl->result['content'] .= '<div id="im_scroll'.$for_user_id.'">';

				if(!$sql_)

					$tpl->result['content'] .= '<div class="info_center"><div style="padding-top:100px">Здесь будет выводиться история переписки.</div></div>';
			}
				...
				foreach($sql_ as $row){
						...
						foreach($attach_arr as $attach_file){
							...
							//Если опрос
								...
								if($vote_id){
									...
									if($row_vote['answer_num']) $answer_num_text = gram_record($row_vote['answer_num'], 'fave');
									else $answer_num_text = $lang['lang_567'];
					...
					if($row['tell_uid']){
						...
						if(date('Y-m-d', $row['tell_date']) == date('Y-m-d', $server_time))
							$dateTell = langdate($lang['lang_643'].' в H:i', $row['tell_date']);
						elseif(date('Y-m-d', $row['tell_date']) == date('Y-m-d', ($server_time-84600)))
							$dateTell = langdate($lang['lang_645'].' в H:i', $row['tell_date']);
						else
							$dateTell = langdate('j F Y в H:i', $row['tell_date']);
...
HTML;
			...
			if(!$first_id){
				$tpl->load_template('im_chat/form.html');
		...
		break;
			...
			case"search":
				...
				$sql_ = $db->super_query("SELECT tb1.friend_id, tb2.user_photo, user_search_pref,user_last_visit FROM `".PREFIX."_friends` tb1,`".PREFIX."_users` tb2 WHERE tb1.user_id = '{$user_id}' AND tb1.friend_id = tb2.user_id AND tb1.subscriptions = 0 AND user_search_pref LIKE '%{$names}%'",1);
				$tpl->load_template('im_chat/dialog.html');
				foreach($sql_ as $row){
					$tpl->set('{name}', $row['user_search_pref']);
					$tpl->set('{uid}', $row['friend_id']);
					if($row['user_photo'])
						$tpl->set('{ava}', '/uploads/users/'.$row['friend_id'].'/50_'.$row['user_photo']);
					else
						$tpl->set('{ava}', '/images/no_ava_50.png');
					if($row['msg_num'])
						$tpl->set('{msg_num}', '<div class="im_new fl_l" id="msg_num'.$row['friend_id'].'">'.$row['msg_num'].'</div>');
					else
						$tpl->set('{msg_num}', '');
					$tpl->compile('dialog');
				}
				echo  $tpl->result['dialog'];
				die();
			break;
			...
			NoAjaxQuery();
			//################### Вывод всех диалогов ###################//
			$sql_ = $db->super_query("SELECT tb1.msg_num, im_user_id, tb2.user_search_pref, user_photo,user_last_visit FROM `".PREFIX."_im` tb1, `".PREFIX."_users` tb2 WHERE tb1.iuser_id = '".$user_id."' AND tb1.im_user_id = tb2.user_id ORDER by `idate` DESC LIMIT 0, 50", 1);
			$tpl->load_template('im_chat/dialog.html');
			$b = 0;
			foreach($sql_ as $row){
			$b++;
				...
				if($row['user_last_visit'] >= $online_time)
					$tpl->set('{online}', 'fc_contact_status');
				else
					$tpl->set('{online}', '');
				$tpl->compile('dialog');
			}
			...
			//header сообщений
			$tpl->load_template('im_chat/head.html');
			$num = $b;
			$words = Array("диалог", "диалога", "диалогов");
			$tpl->set('{count-fr}', $num.' '.num2word($num, $words));
