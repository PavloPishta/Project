...
if($logged){
	...
	$limit_news = 20;
			...
			if($type == 'updates'){
				...
				$user_fm_wrap_bar = $lang['news_fm_wrap_bar'];
				$sql_sort = '4,5';
			...
			//Если вызвана страница фотографий
				...
				$user_fm_wrap_bar = $lang['news_fm_wrap_bar_photos'];
				$sql_sort = '3';
			...
			//Если вызвана страница видео
				...
				$user_fm_wrap_bar = $lang['news_fm_wrap_bar_videos'];
				$sql_sort = '2';
			...
			//Если вызвана страница ответов
				...
				$user_fm_wrap_bar = $lang['news_fm_wrap_bar_notifications'];
				$sql_sort = '6,7,8,9,10';
			...
			//Head
			if(!$_POST['page_cnt']){
				$tpl->load_template('news/head.html');
				$tpl->set('[news]', '');
				$tpl->set('[/news]', '');
				$tpl->set("{activetab-{$type}}", 'page_tabs_active');
				$tpl->set('{type}', $type);
			...
			if($sql_){
				...
				//Если страница "ответов" то загружаем шаблон notifications.html
				if($type == 'notifications')
					$tpl->load_template('news/notifications.html');
				else
					$tpl->load_template('news/news.html');
				
				foreach($sql_ as $row){
				
					if($row['action_type'] != 11){
						$rowInfoUser = $db->super_query("SELECT user_search_pref, user_last_visit, user_photo, user_sex, user_privacy, alias FROM `".PREFIX."_users` WHERE user_id = '{$row['ac_user_id']}'");
					...
					if($rowInfoUser['alias']) {
                        $tpl->set('{alias}', $rowInfoUser['alias']);
                    } else {
                        $tpl->set('{alias}', 'id'.$row['ac_user_id']);
                    }
					} else {
					
					$rowInfoUser = $db->super_query("SELECT title, photo, comments, adres FROM `".PREFIX."_communities` WHERE id = '{$row['ac_user_id']}'");
					
					if($rowInfoUser['adres']) {
                        $tpl->set('{alias}', $rowInfoUser['adres']);
                    } else {
                        $tpl->set('{alias}', 'public'.$row['ac_user_id']);
                    }
							...
							foreach($expFriensList as $fr_id){
								...
								if($fr_info){
									if($fr_info['user_photo'])
										...
										$ava = '/images/no_ava.gif';
										
									$newfriends .= "<div class=\"newsnewfriend\"><a href=\"/id{$fr_id}\" onClick=\"Page.Go(this.href); return false\"><img src=\"{$ava}\" width=\"100px\" alt=\"\" />{$fr_info['user_search_pref']}</a></div>";
								...
								foreach($attach_arr as $attach_file){
									...
									//Если ссылка
										...
										if(stripos($attach_type[4], '/uploads/attach/') === false){
											$attach_type[4] = '/images/no_ava.gif';
											$no_img = false;
										} else
											$no_img = true;
										...
										if($vote_id){
											...
											for($ai = 0; $ai < sizeof($arr_answe_list); $ai++){
												...
												if(!$checkMyVote['cnt']){
													...
													<div class=\"wall_vote_proc fl_l\"><div class=\"wall_vote_proc_bg\" style=\"width:".intval($proc)."%\"></div><div style=\"margin-top:-21px\">{$num}</div></div>
													<div class=\"fl_l\" style=\"margin: 3px 0;\"><b>{$proc}%</b></div>
...
HTML;
					...
					//Если страница ответов "мне нравится"
						...
						$row_info = $db->super_query("SELECT id, author_user_id, for_user_id, text, add_date, tell_uid, tell_date, type, public, attach, tell_comm FROM `".PREFIX."_wall` WHERE id = '{$row['obj_id']}'");
						if($row_info){
							$str_text = strip_tags(substr($row_info['text'], 0, 70, 'utf-8'));
...
HTML;
							...
							$likesUseList = explode('|', str_replace('id', '', $row['action_text']));
							$rList = '';
							$uNames = '';
							$cntUse = 0;
...
HTML;
						...
						//Мне нравится
						if(stripos($rec_info['likes_users'], "id{$user_id}|") !== false){
							$tpl->set('{yes-like}', 'str_wall_like_yes');
							$tpl->set('{yes-like-color}', 'str_wall_like_yes_color');
...
HTML;
						...
						//Мне нравится
						if(stripos($rec_info_groups['likes_users'], "id{$user_id}|") !== false){
							$tpl->set('{yes-like}', 'str_wall_like_yes');
							$tpl->set('{yes-like-color}', 'str_wall_like_yes_color');
