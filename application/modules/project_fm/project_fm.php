...
if($logged){
	...
	$metatags['title'] = 'Project FM';

	$user_fm_wrap_bar = 'Project FM';
...
$go_back = '<a href="/project_fm" onclick="Page.Go(this.href); return false;">Вернуться назад</a>';
...
foreach($db_afisha as $views_afisha){
	...
	$tpl->load_template('project_fm/right_content.html');
...
foreach($db_news as $views_news){
	...
	$tpl->load_template('project_fm/left_content.html');
	...
	$tpl->set('{all_text}', '<a href="/project_fm/news_wiev_all/id/'.$views_news['id'].'" onclick="Page.Go(this.href); return false;">Читать дальше</a>');
...
if ($act == 'news_all'){
	...
	$tpl->load_template('project_fm/index_fm.html');
...
case "news_all":
	...
	foreach($rownews_all as $row_news_all){
		...
		$tpl->load_template('project_fm/news_all.html');
		...
		navigation($gcount, $db_cnt['id'], $config['home_url'].'project_fm/news_all/page/');
...
case "news_wiev_all":
	...
	$tpl->load_template('project_fm/news_wiev_all.html');
...
case "news_adds":
	...
	if($group == 1){
		...
		$tpl->load_template('project_fm/news_adds.html');
...
case "news_edit_admin":
	...
	$row = $db->super_query("SELECT id, title, news, photos FROM `".PREFIX."_pfm_news` WHERE id = '{$id}'");
		...
		$tpl->load_template('project_fm/news_edit.html');
...
case "chat_index":
	...
	$tpl->load_template('project_fm/chat_index.html');
...
case "chat_fm_view":
	...
	if($update_chat){
		...
		foreach($update_chat as $row_upd){
			...
			$tpl->load_template('project_fm/chat_message.html');
...
case "chat_fm_user":
	...
	$chat_user = $db->super_query("SELECT tb1.id, tb1.user_id, tb2.user_photo, tb2.user_search_pref, tb2.alias FROM `".PREFIX."_pfm_act` tb1,`".PREFIX."_users` tb2 WHERE tb1.user_id = tb2.user_id", 1);
		...
		foreach($chat_user as $row_active){
			...
			$tpl->load_template('project_fm/chat_user_act.html');
...
case "afisha_adds":
	...
	if($group == 1){
		...
		$tpl->load_template('project_fm/afisha_adds.html');
...
case "afisha_edit_admin":
	...
	$row = $db->super_query("SELECT id, title, description, place, date, sponsor, photos FROM `".PREFIX."_pfm_afisha` WHERE id = '{$id}'");
		...
		$tpl->load_template('project_fm/afisha_edit.html');
...
case "afisha_all":
	...
	foreach($row as $row_afisha_all){
		...
		$tpl->load_template('project_fm/afisha_all.html');
		...
		navigation($gcount, $db_cnt['id'], $config['home_url'].'project_fm/afisha_fm/page/');
