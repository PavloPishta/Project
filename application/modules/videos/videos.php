...
if($logged){
	...
	switch($act){
		...
		case "add":
			NoAjaxQuery();
			$tpl->load_template('videos/add.html');
			$tpl->compile('content');
			AjaxTpl();
			die();
		break;
		...
		case "send":
			...
			if($config['video_mod_add'] == 'yes'){
				...
				//Если vk
				if(preg_match("/src=\"http:\/\/www.vk.com|src=\"http:\/\/vk.com/i", 'src="'.$good_video_lnk)){
					$result_video_lnk = '<iframe src="'.$good_video_lnk.'" width="770" height="420" frameborder="0"></iframe>';
				}
		...
		//################### Страница добавления видео ###################//
		case "upload_add":
			$tpl->load_template('videos/upload.html');
			$tpl->compile('content');
			AjaxTpl();
			die();
		break;
		...
		case "load":
			...
			$video_lnk = str_replace('<iframe src="', '', $video_lnk);
			$exp1_lnk = explode('"', $video_lnk);
			$video_lnk = $exp1_lnk[0];

			if(preg_match("/http:\/\/www.youtube.com|http:\/\/youtube.com|http:\/\/rutube.ru|http:\/\/www.rutube.ru|http:\/\/www.vimeo.com|http:\/\/vimeo.com|http:\/\/smotri.com|http:\/\/www.smotri.com|http:\/\/www.vk.com|http:\/\/vk.com/i", $video_lnk)){
				...
				if(!$sock){
					...
					if(preg_match("/http:\/\/www.vk.com|http:\/\/vk.com/i", $video_lnk)){
						...
						$html = file_get_contents($video_lnk);
		...
		case "edit":
			...
			if($vid){
				...
				if($row){
					$tpl->load_template('videos/editpage.html');
		...
		case "view":
			NoAjaxQuery();
			include APPLICATION_DIR.'/modules/videos/video.php';
			die();
		break;
			...
			if($config['video_mod_comm'] == 'yes'){
				...
				if(!$CheckBlackList){
					if($check_video AND isset($comment) AND !empty($comment)){
						...
						$tpl->load_template('videos/comment.html');
		...
		case "all_videos":
			...
			if($count['user_videos_num']){
				if($notes)
					$tpl->load_template('videos/box_all_video_notes_top.html');
				else
					$tpl->load_template('videos/box_all_video_top.html');
				...
				if(!$notes)
					$tpl->load_template('videos/box_all_video.html');
				else
					$tpl->load_template('videos/box_all_video_notes.html');
				...
				$tpl->load_template('albums_editcover.html');
				$tpl->set('[bottom]', '');
				$tpl->set('[/bottom]', '');
		...
		case "page":
			...
			if(!$CheckBlackList){
				if($last_id){
					...
					//SQL Запрос
					$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS id, title, photo, comm_num, add_date FROM `".PREFIX."_videos` WHERE owner_user_id = '{$get_user_id}' AND id < '{$last_id}' {$sql_privacy} ORDER by `add_date` DESC LIMIT 0, {$limit_vieos}", 1);
					
					//Если есть ответ из БД
					if($sql_){
						$tpl->load_template('videos/short.html');
						foreach($sql_ as $row){
		...
		case "addmylist":
			...
			if($row AND $config['video_mod_add_my'] == 'yes'){
				...
				$db->query("INSERT INTO `".PREFIX."_videos` SET owner_user_id = '{$user_id}', video = '{$row['video']}', photo = '{$newPhoto}', title = '{$row['title']}', descr = '{$row['descr']}', add_date = NOW(), privacy = 1, nocopy = 1");
				$dbid = $db->insert_id();
				$db->query("UPDATE `".PREFIX."_users` SET user_videos_num = user_videos_num+1 WHERE user_id = '{$user_id}'");
		...
		default:
			...
			if(!$CheckBlackList){
				...
				if($owner){
					...
					if($get_user_id == $user_id)
						$user_fm_wrap_bar = 'У Вас <span id="nums">'.($owner['user_videos_num'] ? $owner['user_videos_num'] : false).'</span> '.gram_record($owner['user_videos_num'], 'videos');
					else
						$user_fm_wrap_bar = 'У '.gramatikName($name_info[0]).' '.($owner['user_videos_num'] ? $owner['user_videos_num'] : false).' '.gram_record($owner['user_videos_num'], 'videos');
					
					if($owner['user_videos_num']){
					
						//SQL Запрос
						$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS id, title, photo, photo_original, comm_num, add_date, SUBSTRING(descr, 1, 180) AS descr FROM `".PREFIX."_videos` WHERE owner_user_id = '{$get_user_id}' {$sql_privacy} ORDER by `add_date` DESC LIMIT 0, {$limit_vieos}", 1);

						//Загружаем меню по видео
						$tpl->load_template('videos/head.html');
						$tpl->set('{user-id}', $get_user_id);
						...
						if($sql_){
							...
							foreach($sql_ as $row){
								...
								if($row['photo_original']){
									$tpl->set('{photo}', stripslashes($row['photo_original'])); 
								} else {
									$tpl->set('{photo}', stripslashes($row['photo']));
								}
							...
							$tpl->load_template('videos/bottom.html');
							$tpl->compile('content');
