...
if($logged){
	...
	switch($act){
		...
		case "send":
			NoAjaxQuery();
			$wall_text = htmlspecialchars_decode(ajax_utf8(textFilter($_POST['wall_text'])));
						...
						if($xPrivasy){
							...
							if($answer_comm_id){
								...
								if($user_id != $row_owner2['author_user_id'] AND $row_owner2){
									...
									$wall_text = str_replace($check2['user_name'], "<a href=\"/id{$row_owner2['author_user_id']}\" onClick=\"Page.Go(this.href); return false\" style=\"color:#666\">{$check2['user_name']}</a>", $wall_text);
							...
							include APPLICATION_DIR.'/classes/wall.php';
							$wall = new wall();
							...
							if(!$fast_comm_id){
								...
								if($xPrivasyX){
									...
									$wall->template('wall/record.html');
							...
							//Если добавлен комментарий к записи то просто обновляем нужную часть, тоесть только часть комментариев, но не всю стену
								...
								if($_POST['type'] == 1)
									$wall->comm_template('news/news.html');
								else if($_POST['type'] == 2)
									$wall->comm_template('wall/one_record.html');
								else
									$wall->comm_template('wall/record.html');
		...
		case "delet":
			...
			if($row['author_user_id'] == $user_id OR $row['for_user_id'] == $user_id){
				...
				if(!$row['fast_comm_id']){
					...
					if(stripos($row['attach'], 'graffiti|') !== false){
                      $attach_arr = explode('graffiti|', $row['attach']);
					if(file_exists(ROOT_DIR.$attach_arr[1]))
						@unlink(ROOT_DIR.$attach_arr[1]);       
					}
		...
		case "like_yes":
			...
			if($row){
				...
				$likes_users = explode('|', str_replace('id', '', $row['likes_users']));
				if(!in_array($user_id, $likes_users)){
					$db->query("INSERT INTO `".PREFIX."_wall_like` SET rec_id = '{$rid}', user_id = '{$user_id}', date = '{$server_time}'");

					$db->query("UPDATE `".PREFIX."_wall` SET likes_num = likes_num+1, likes_users = '|id{$user_id}|{$row['likes_users']}' WHERE id = '{$rid}'");
					...
					//Вставляем событие в моментальные оповещания
						...
						if($row_news)
							$db->query("UPDATE `".PREFIX."_news` SET action_text = '|id{$user_id}|{$row_news['action_text']}', action_time = '{$server_time}' WHERE obj_id = '{$rid}' AND action_type = 7 AND action_time = '{$row_news['action_time']}'");
						else
							$db->query("INSERT INTO `".PREFIX."_news` SET ac_user_id = '{$user_id}', action_type = 7, action_text = '|id{$user_id}|', obj_id = '{$rid}', for_user_id = '{$row['author_user_id']}', action_time = '{$server_time}'");
					}
				}
			}
		...
		case "like_no":
			...
			if($row){
				...
				if(in_array($user_id, $likes_users)){
					...
					$newListLikesUsers = strtr($row['likes_users'], array('|id'.$user_id.'|' => ''));
					...
					$row_news['action_text'] = strtr($row_news['action_text'], array('|id'.$user_id.'|' => ''));
		...
		case "liked_users":
			...
			if($sql_){
				foreach($sql_ as $row){
					...
					echo '<a href="/id'.$row['user_id'].'" id="Xlike_user'.$row['user_id'].'_'.$rid.'" onClick="Page.Go(this.href); return false"><img src="'.$ava.'" width="32" /></a>';
				}
			}
			die();
		break;
		...
		case "parse_link":
			...
			if(stripos(strtolower($open_lnk), 'charset=utf-8') OR stripos(strtolower($check_url[2]), 'charset=utf-8'))
				...
				$open_lnks = ajax_utf8($open_lnk);
				
				//$open_lnk = iconv("UTF-8","CP1251//IGNORE", $open_lnk);
			...
			default:
				...
				if(!$id){
					...
					$user_fm_wrap_bar = 'На стене нет записей';
					...
					if($row_user){
						...
						if(!$CheckBlackList){
							...
							if($cnt_rec['cnt'] > 0)
								$user_fm_wrap_bar = 'На стене '.$cnt_rec['cnt'].' '.gram_record($cnt_rec['cnt'], 'rec');

							$tpl->load_template('wall/head.html');
							$tpl->set('{name}', gramatikName($row_user['user_name']));
							$tpl->set('{uid}', $id);
							$tpl->set('{rec-id}', $rid);
							$tpl->set("{activetab-{$_GET['type']}}", 'page_tabs_active');
							$tpl->compile('info');
				...
				if(!$CheckBlackList){
					...
					if(!$Hacking){
						if($rid OR $walluid){
							$wall->template('wall/one_record.html');
							$wall->compile('content');
							$wall->select();
