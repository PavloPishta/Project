...
if($logged){
	...
	switch($act){
		...
		case "search":
			...
			$tpl->load_template('public_audio/search_result.html');
			...
			if($sql_){
				...
				if(!$page_cnt)
					$tpl->result['content'] .= "<script>langNumric('langNumric', '{$row_count['cnt']}', 'аудиозапись', 'аудиозаписи', 'аудиозаписей', 'аудиозапись', 'аудиозаписей');</script><div id=\"page_rows_summary\" class=\"summary_wrap\"><div class=\"summary\">В поиске найдено <span id=\"seAudioNum\">{$row_count['cnt']}</span> <span id=\"langNumric\"></span> | <a href=\"/{$adres}\" onClick=\"Page.Go(this.href); return false\" style=\"font-weight:normal\">К сообществу</a> | <a href=\"/\" onClick=\"Page.Go(location.href); return false\" style=\"font-weight:normal\">Все аудиозаписи</a></div></div>";
				...
				if(!$page_cnt){
					...
					$tpl->result['info'] .= "<div id=\"page_rows_summary\" class=\"summary_wrap\"><div class=\"summary\">Нет аудиозаписей | <a href=\"/{$adres}\" onClick=\"Page.Go(this.href); return false\" style=\"font-weight:normal\">К сообществу</a> | <a href=\"/\" onClick=\"Page.Go(location.href); return false\" style=\"font-weight:normal\">Все аудиозаписи</a></div></div>";
		...
		default:
			...
			if(!$page_cnt){
				$tpl->load_template('public_audio/top.html');
				$tpl->set('{pid}', $pid);
					
				if($infoGroup['adres']) $tpl->set('{adres}', $infoGroup['adres']);
				else $tpl->set('{adres}', 'public'.$pid);
						
				if($infoGroup['audio_num']) $tpl->set('{audio-num}', $infoGroup['audio_num'].' <span id="langNumric"></span>');
			...
			if($sql_){
				...
				$tpl->load_template('public_audio/track.html');
				...
				if($infoGroup['audio_num'] > $sql_limit AND !$page_cnt)
					$tpl->result['content'] .= '<div id="ListAudioAddedLoadAjax"></div><div class="cursor_pointer" style="margin-top:-4px" onClick="ListAudioAddedLoadAjax()" id="wall_l_href_se_audiox"><div class="se_more profile_hide_opne" style="width:652px" id="wall_l_href_audio_se_loadx">Показать больше аудиозаписей</div></div>';
