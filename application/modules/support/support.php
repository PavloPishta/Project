...
if($logged){
	...
	switch($act){
		...
		case "new":
			$tpl->load_template('support/new.html');
			$tpl->set('{uid}', $user_id);
			$tpl->compile('content');
		break;
		...
		case "send":
			...
			if(!$rowLast['cnt'] AND isset($title) AND !empty($title) AND isset($question) AND !empty($question) AND $user_info['user_group'] == 5 ){
				...
				$tpl->load_template('support/show.html');
		...
		case "delet":
			...
			if($row['suser_id'] == $user_id OR $user_info['user_group'] == 4  OR $user_info['user_group'] ==1){
		...
		case "delet_answer":
			...
			if($row['auser_id'] == $user_id OR $user_info['user_group'] == 4  OR $user_info['user_group'] ==1)
		...
		case "close":
			NoAjaxQuery();
			$qid = intval($_POST['qid']);
			if($user_info['user_group'] == 4  OR $user_info['user_group'] ==1){
		...
		case "answer":
			...
			if($check['suser_id'] == $user_id OR $user_info['user_group'] == 4  OR $user_info['user_group'] ==1 AND isset($answer) AND !empty($answer)){
				if($user_info['user_group'] == 4  OR $user_info['user_group'] ==1){
					$auser_id = 0;
				...
				$tpl->load_template('support/answer.html');
				if(!$auser_id){
				...
				if($auser_id == $user_id OR $user_info['user_group'] == 4  OR $user_info['user_group'] ==1){
					$tpl->set('[owner]', '');
					$tpl->set('[/owner]', '');
				} else
		...
		case "show":
			...
			if($row){
				...
				foreach($sql_answer as $row_answer){
					...
					if($row_answer['auser_id'] == $user_id OR $user_info['user_group'] == 4  OR $user_info['user_group'] ==1){
						$tpl->set('[owner]', '');
						$tpl->set('[/owner]', '');
					} else
						$tpl->set_block("'\\[owner\\](.*?)\\[/owner\\]'si","");
		...
		break;
			...
			default:
				if($user_info['user_support'] AND $user_info['user_group'] == 5 )
				...
				$tpl->load_template('support/head.html');
				if($sql_)
					if($user_info['user_group'] == 4  OR $user_info['user_group'] ==1)
				...
				if($sql_){
					$tpl->load_template('support/question.html');
					foreach($sql_ as $row){
						$tpl->set('{title}', stripslashes($row['title']));
						megaDate($row['sdate']);
						if($row['sfor_user_id'] == $row['suser_id'] OR $user_info['user_group'] == 4  OR $user_info['user_group'] ==1){
							if($row['sfor_user_id'] == $row['suser_id'])
								$tpl->set('{status}', 'Вопрос ожидает обработки.');
							else
								$tpl->set('{status}', 'Есть ответ.');
