	...
	switch($act){
		...
		default:
			...
			if($user['user_fave_num']){
				...
				$user_fm_wrap_bar = '<span id="fave_num">'.$user['user_fave_num'].'</span> '.gram_record($user['user_fave_num'], 'fave');
				
				//Загружаем поиск на странице
				$tpl->load_template('fave_search.html');
				$tpl->compile('content');
