...
var reg = {
	...
	send: function(sec_code){
		...
		$.post('/index.php?go=register', {
			...
			var exp = d.split('|');
			if(exp[0] == 'ok'){
				window.location = '/id'+exp[1]+'&news_photo_load';
...
function isValidName(xname){
	var pattern = new RegExp(/^[a-zA-Zа-яёА-ЯЁ]+$/);
...
function checkCode(){
	...
	$.get('/captcha/sec_code.php?user_code='+val_sec_code, function(data){
		if(data == 'ok'){
