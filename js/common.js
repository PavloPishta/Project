...
var groups = {
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('http://koftagard.ru/id', '');
	...
	send_new_admin: function(id, new_admin_id){
		...
		var data = '<div class="facem_groups_oneadmin" id="admin'+new_admin_id+'"><a href="/id'+new_admin_id+'" onClick="Page.Go(this.href); return false"><img src="'+ava+'" align="left" width="32" /></a><a href="/id'+new_admin_id+'" onClick="Page.Go(this.href); return false">'+adm_name+'</a><br /><a href="/" onClick="groups.deladmin(\''+id+'\', \''+new_admin_id+'\'); return false"><small>Удалить</small></a></div>';
...
'<div class="photo_close" onClick="Photo.Close(\'\'); return false;"></div>'+
 '<div class="photo_bg" style="min-height:400px"><div class="newphbg" style="margin-bottom:20px">'+
  '<div class="photo_com_title" style="padding-top:0px;">'+topTxt+'<div><a href="/" onClick="Photo.Close(\'\'); return false">Закрыть</a></div></div>'+
  '<div class="photo_img_box cursor_pointer" onClick="'+next+'"><img src="'+photo+'" id=\"photo_view_src\" style="margin-bottom:7px" /></div></div><div class="line_height">'+
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на KoftaGard', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи на KoftaGard.."  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_cancel, 'Прикрпепить', 'groups.wall_video_add_select()');
...
var audio = {
	addBox: function(){
		Box.Close();
		Box.Show('addaudio', 525, lang_audio_add, '<div class="tags_box"><div id="page_tabs_bar"><div class="onclic_botton cursor_pointer"><a><div><b>По ссылке</b></div></a></div><a class="cursor_pointer" onClick="audio.addBoxComp()"><div><b>С компьютера</b></div></a><div class="clear"></div></div><div id="tags_box_content"><div class="videos_text">Вставьте ссылку на mp3 файл</div><input type="text" class="videos_input" id="audio_lnk" style="margin-top:5px" /><span id="vi_info">Например: <b>http://music.com/uploads/files/audio/2012/faxo_-_kalp.mp3</b></span></div></div>', lang_box_cancel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
		$('#audio_lnk').focus();
	},
	addBoxComp: function(){
		Box.Close();
		Box.Show('addaudio_comp', 525, lang_audio_add, '<div class="tags_box"><div id="page_tabs_bar"><a onClick="audio.addBox()" class="cursor_pointer"><div><b>По ссылке</b></div></a><div class="onclic_botton cursor_pointer"><a><div><b>С компьютера</b></div></a></div></div><div id="tags_box_content"><div class="videos_text">Ограничения<div class="clear"></div><li style="font-weight:normal;color:#000;font-size:11px;margin-top:10px">Аудиофайл не должен превышать 100 Мб и должен быть в формате MP3.</li><li style="font-weight:normal;color:#000;font-size:11px;margin-bottom:15px">Аудиофайл не должен нарушать авторские права.</li><div class="button_div fl_l" style="margin-left:185px"><button id="upload">Выбрать файл</button></div><div class="clear"></div><div style="margin-top:15px;font-size:11px;color:#000;font-weight:normal">Вы также можете добавить аудиозапись из числа уже загруженных файлов, воспользовавшись <a href="/?go=search&type=5"><b>поиском по аудио.</b></a></div></div></div></div>', lang_box_cancel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
		$('#audio_lnk').focus();
		$('#box_but').hide();
...
var projectfm = {
	afisha_save: function(){
		...
		butloading('sending', '56', 'disabled', '');
		$.post('/index.php?go=project_fm&act=afisha_save', {title: title, description: description, place: place, date: date, sponsor: sponsor, photos: photos}, function(d){
			if(d == '1'){
	...
	afisha_delete: function(id){
		$('#afisha_'+id).html('<div style="padding:10px;color:#666;"><center>Афиша удалено успешно.</center></div><div class="clear"></div>');
			$.post('/index.php?go=project_fm&act=afisha_delete&id='+id, {id: id});
	},
	afisha_edit: function(id){
		...
		var sponsor = $('#sponsor').val();
		var photos = $('#photos').val();
		$.post('/index.php?go=project_fm&act=afisha_edit_save&id='+id, {id: id, title: title, description: description, place: place, date: date, sponsor: sponsor, photos: photos}, function(d){
	...
	chat_fm_message: function(){
		...
		butloading('sending', '56', 'disabled', '');
		$.post('/index.php?go=project_fm&act=chat_fm_message', {chat_message: chat_message}, function(d){
	...
	news_save: function(){
		...
		butloading('sending', '56', 'disabled', '');
		$.post('/index.php?go=project_fm&act=news_save', {title: title, news: news, photos: photos}, function(d){
			if(d == '1'){
				var result = 'Сохранено успешно.';    
			}
			else if(d == '2'){
	...
	news_edit_save: function(id){
		...
		var title = $('#title').val();
		var photos = $('#photos').val();
		$.post('/index.php?go=project_fm&act=news_edit_save&id='+id, {title: title, news: news, photos: photos}, function(d){
	...
	news_delete: function(id){
		$('#news_'+id).html('<div style="padding:10px;color:#666;"><center>Новость удалено успешно.</center></div><div class="clear"></div>');
			$.post('/index.php?go=project_fm&act=news_delete&id='+id, {id: id});
	},
}